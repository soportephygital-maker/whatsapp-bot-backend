...
def _sender_context(db: Session, request: HelpRequest):
    ...
    if not store and request.company_id:
        store = db.query(Store).filter(Store.company_id == request.company_id, Store.whatsapp_phone_number_id.isnot(None)).order_by(Store.id.asc()).first()
...
def _notify_contact(db: Session, request: HelpRequest, company: Company, store: Store | None, sender_id: str | None, contact: SupportContact, action: str, stage: str) -> bool:
    try:
        result = send_text_message(contact.phone, _support_message(company, store, request, stage), phone_number_id=sender_id, db=db)
    ...
    if not result.get('sent'):
        db.add(AuditLog(action='support_notification_blocked', entity='help_request', entity_id=str(request.id), details={'support_id': contact.id, 'role': contact.role, 'reason': result.get('reason')}))
        return False
    db.add(AuditLog(action=action, entity='help_request', entity_id=str(request.id), details={'support_id': contact.id, 'support_name': contact.name, 'support_phone': contact.phone, 'role': contact.role}))
    return True
    ...
    for request in requests:
        ...
        if not primary_log:
            ...
            if emit_notification(db, audience='operator', event_type='escalation_started', title=f'Escalamiento iniciado - {store_name}', body=f'La solicitud #{request.id} de {company.name} lleva 5 minutos sin respuesta humana.', event_key=f'help:{request.id}:operator:escalation_started', details={'help_request_id': request.id, 'company': company.name, 'store': store_name}):
                app_events += 1
            if primary:
                if _notify_contact(db, request, company, store, sender_id, primary, 'support_primary_notified', 'primario'):
                    sent += 1
                else:
                    blocked += 1
            else:
        ...
        if primary_log and not secondary_log:
            ...
            if _elapsed_minutes(primary_log.created_at, now) < wait_primary:
                continue
            if secondary:
                if _notify_contact(db, request, company, store, sender_id, secondary, 'support_secondary_notified', 'secundario'):
                    sent += 1
                else:
                    blocked += 1
                db.flush()
