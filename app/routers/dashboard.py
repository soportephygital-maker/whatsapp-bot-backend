...
def notifications(
    ...
    rows = db.query(AppNotification).filter(
        AppNotification.id > after_id,
        AppNotification.audience.in_(audiences),
    ).order_by(AppNotification.id.asc()).limit(100).all()
    return [{
        'id': row.id,
...
def reply_conversation(conversation_id: int, data: ConversationReply, user: User = Depends(require_operator), db: Session = Depends(get_db)):
    ...
    if not result.get('sent'):
        db.add(AuditLog(username=user.username, action='manual_reply_blocked', entity='conversation', entity_id=str(conversation.id), details={'to': conversation.wa_user_id, 'reason': result.get('reason') or result.get('error')}))
        db.commit()
...
def help_requests(status: str | None = Query(default=None), company_id: int | None = Query(default=None), _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    companies = {c.id: c.name for c in db.query(Company).all()}
    channel_map = {c.conversation_id: c for c in db.query(ConversationChannel).all()}
    stores = {s.id: s.name for s in db.query(Store).all()}
    return [{
        'id': r.id,
        'company_id': r.company_id,
        'company_name': companies.get(r.company_id, 'Sin empresa'),
        'store_name': stores.get(channel_map.get(r.conversation_id).store_id, 'Tienda sin identificar') if channel_map.get(r.conversation_id) else 'Tienda sin identificar',
...
def update_help_request(request_id: int, data: HelpRequestStatus, user: User = Depends(require_operator), db: Session = Depends(get_db)):
    ...
    if data.status in ('resolved', 'ignored') and previous not in ('resolved', 'ignored'):
        ...
        outcome = 'atendido con éxito' if success else 'cerrado sin atención exitosa'
        for audience in ('admin', 'reader'):
            emit_notification(
                db,
                audience=audience,
                event_type='help_request_closed',
                title=f'Caso cerrado - {store_name}',
                body=f'Solicitud #{row.id} de {company_name}: {outcome}.',
                event_key=f'help:{row.id}:{audience}:closed:{data.status}',
...
def audit_ui_event(event: UIAuditEvent, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    db.add(AuditLog(username=user.username, action=event.action, entity='ui', details={'element_id': event.element_id, 'label': event.label, 'path': event.path}))
    db.commit()
    return {'status': 'ok'}
