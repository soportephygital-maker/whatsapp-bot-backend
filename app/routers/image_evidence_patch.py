...
def _contact_identity(db: Session, conversation: Conversation, data: local_bridge.LocalInbound) -> tuple[str, str]:
    ...
    if phone_digits:
        for row in db.query(Contact).filter(Contact.is_active.is_(True)).all():
            if _digits(row.phone).endswith(phone_digits[-10:]) or phone_digits.endswith(_digits(row.phone)[-10:]):
                name = str(row.display_name or '').strip()
                if name:
                    break
    if not name and display and _digits(display) != _digits(phone):
        name = display
    if not name and sender and _digits(sender) != _digits(phone):
        name = sender
...
def _incident_reason(ticket: SupportTicket | None) -> str:
    ...
    match = re.search(r'(?im)^\s*Cómo sucedió\s*:\s*(.+)    db: Session,
    *,
    operator: User,
...
def _handle_confirmation_reply(
    ...
    *,
    ...
    # A correct photo no longer closes the ticket immediately. First show the
    # complete report summary so the user can verify or correct the data.
    conversation.status = 'open'
...
def _fallback_report_reason(ticket: SupportTicket | None) -> str:
    ...
    cleaned = re.sub(r'(?im)Cómo sucedió\s*:\s*.+    db: Session,
    *,
    operator: User,
    conversation: Conversation | None,
    ticket: SupportTicket | None,
...
def _report_review_data(
    ...
    *,
    ...
    base.update({k: v for k, v in overrides.items() if k in base and str(v).strip()})
    return base
...
def _handle_report_review_reply(
    ...
    *,
    ...
    if answer in {'1', 'si', 'correcto', 'correcta', 'esta bien', 'confirmar'}:
        ...
        db.add(AuditLog(
            username=operator.username,
            action='image_report_confirmed_open_review',
            entity='conversation',
            entity_id=str(conversation.id),
            details={'ticket_id': ticket.id if ticket else None},
        ))
        db.commit()
        return {
            'status':'ok','conversation_id':conversation.id,'company_key':company.company_key,
...
def image_evidence_inbound(
    ...
    if conversation:
        ...
        db.commit()
    return global_entry_sequence_patch.global_entry_sequence_inbound(data=data, operator=operator, db=db)
, '', str(ticket.description or '')).strip()
