...
def _html() -> str:
    ...
    html = html.replace('</head>', '''<style id="dashboardGeneralNavigation77">
#navGeneral{font-weight:700!important}.dash-action{cursor:pointer;transition:transform .16s ease,filter .16s ease,box-shadow .16s ease}.dash-action:hover{transform:translateY(-2px);filter:brightness(1.08);box-shadow:0 10px 26px rgba(0,0,0,.18)}.dash-action:focus{outline:2px solid #53aaff;outline-offset:2px}.dash-general-welcome{padding:16px 18px;margin-bottom:14px}.dash-general-welcome h2{margin:0 0 5px;font-size:24px}.dash-general-welcome p{margin:0;color:#8eacc8}.ai-node-editor{display:grid;gap:9px;margin-top:12px;padding:12px;border:1px solid #62431f;border-radius:12px;background:#0b0907}.ai-node-editor label{font-size:11px;color:#c9ad8d}.ai-node-editor textarea{min-height:88px}.ai-node-editor input[type=number]{max-width:130px}.ai-node-danger{background:#4a1920!important;border-color:#9d3e4c!important;color:#ffdce2!important}.ai-manager-note{margin:10px 0 0;padding:10px 12px;border-radius:11px;background:#102c43;border:1px solid #26577b;color:#d7eaf8;font-size:12px}.ai-manager-note b{color:#fff}.ai-manager-approve{background:#0c5b42!important;border-color:#188966!important;color:#e0fff4!important}
</style></head>''')
    return html
...
const _aiNavOriginal=typeof installSuperAdminAiNav==='function'?installSuperAdminAiNav:null;
installSuperAdminAiNav=function(){
  ...
  btn=document.createElement('button');btn.id='navAINeural';btn.className='ps-ai-nav';btn.type='button';
  btn.innerHTML='<span>✦</span> <span>IA · Aprendizaje</span>';
...
function renderManagerNeuronDetail(point){
  ...
  if(!point){box.innerHTML='<div class="muted">Selecciona una neurona para revisar el aprendizaje. Como Gerente puedes aprobar los puntos pendientes.</div>';return}
...
async function renderManagerAiNeural(){
  ...
  if(typeof LIVE_CHAT_ID!=='undefined')LIVE_CHAT_ID=null;
  try{sessionStorage.setItem('phygital_dashboard_view_v1','ai')}catch(_){}
  err('');
  const content=document.getElementById('content');if(!content)return;
  content.innerHTML='<div class="muted">Cargando aprendizaje visible para Gerente...</div>';
  try{
    const s=await api('/api/admin-ai/status');
    const points=s.recent_points||[];
    content.innerHTML=`<div class="ai-neural-head"><div class="ai-neural-title"><div class="ai-core-icon">✦</div><div><h2 style="margin:0">IA · Aprendizaje</h2><div class="muted">Visualiza cómo aprende el sistema y aprueba nuevos puntos de conocimiento.</div></div></div><div class="ai-ai-badge">● Vista Gerente · aprobación</div></div><div class="ai-manager-note"><b>Permisos de Gerente:</b> puedes visualizar todas las neuronas dentro de tus empresas asignadas y aprobar aprendizajes pendientes. No puedes entrenar por chat, editar, rechazar ni eliminar nodos.</div><div class="ai-neural-stats"><div class="ai-neural-stat"><b>${Number(s.score||0)}%</b><span>Nivel ${esc(s.level||'Inicial')}</span></div><div class="ai-neural-stat"><b>${Number(s.approved_points||0)}</b><span>Consolidadas</span></div><div class="ai-neural-stat"><b>${Number(s.pending_points||0)}</b><span>Pendientes de aprobación</span></div><div class="ai-neural-stat"><b>${Number(s.companies_with_learning||0)}</b><span>Empresas aprendidas</span></div></div><div class="ai-neural-shell"><div class="card ai-neural-card"><div class="section-title"><div><h3>Mapa neuronal de aprendizaje</h3><div class="muted">Brillo intenso = aprobado · pulso ámbar = pendiente · tenue = rechazado</div></div><button id="aiRefreshMap">Actualizar mapa</button></div><div class="ai-neural-stage"><svg id="aiNeuralSvg" viewBox="0 0 860 535">${buildNeuralSvg(points,s)}</svg></div></div><div><div class="ai-neuron-detail" id="aiNeuronDetail"><div class="muted">Selecciona una neurona para revisar el aprendizaje.</div></div><div class="card" style="margin-top:12px;padding:12px"><h3 style="margin-top:0">Puntos de aprendizaje</h3><div class="ai-learning-list">${points.map(p=>`<div class="ai-learning-row" data-learning-id="${p.id}"><div class="ai-learning-num">N${p.id}</div><div><b>${esc((p.problem||'').slice(0,95))}</b><small>${esc(p.status)} · confianza ${Number(p.confidence||0)}%</small></div>${p.status==='pending'?'<span class="badge">Revisar</span>':'<span class="badge">Ver</span>'}</div>`).join('')||'<div class="muted">Aún no hay puntos de aprendizaje visibles.</div>'}</div></div></div></div>`;
    window.__managerAiPoints=points;
    const selectPoint=id=>{const p=points.find(x=>String(x.id)===String(id));if(p)renderManagerNeuronDetail(p)};
    content.querySelectorAll('[data-neuron-id]').forEach(el=>el.addEventListener('click',()=>selectPoint(el.dataset.neuronId)));
    content.querySelectorAll('[data-learning-id]').forEach(el=>el.addEventListener('click',()=>selectPoint(el.dataset.learningId)));
    const refresh=document.getElementById('aiRefreshMap');if(refresh)refresh.onclick=renderManagerAiNeural;
    markDashboardNavActive('navAINeural');
  }catch(x){content.innerHTML=`<div class="danger">${esc(x.message)}</div>`}
}

const _superAdminAiRenderer=typeof renderSuperAdminAiNeural==='function'?renderSuperAdminAiNeural:null;
if(_superAdminAiRenderer){
  renderSuperAdminAiNeural=async function(...args){
    if(isManagerAiViewer())return renderManagerAiNeural();
    return _superAdminAiRenderer(...args);
  };
}
...
function wireOverviewShortcuts(){
  ...
  const actions=[
    ...
    {run:()=>canViewAiLearning()&&typeof renderSuperAdminAiNeural==='function'?renderSuperAdminAiNeural():null,nav:'navAINeural',label:'Abrir IA Aprendizaje'},
  ];
  cards.forEach((card,i)=>{
    const action=actions[i];if(!action||card.dataset.navReady==='1')return;
    if(i===3&&!canViewAiLearning()){card.style.display='none';return}
...
function wireSidebarNavigation(){
  ...
  Object.entries(map).forEach(([id])=>{
    const b=document.getElementById(id);if(!b||b.dataset.nav77==='1')return;b.dataset.nav77='1';
...
function renderEditableNeuronDetail(point){
  ...
  if(edit)edit.onclick=()=>editor.style.display='grid';if(cancel)cancel.onclick=()=>editor.style.display='none';if(approve)approve.onclick=()=>updateNeuralLearning(point.id,'approved');if(reject)reject.onclick=()=>updateNeuralLearning(point.id,'rejected');
  if(save)save.onclick=async()=>{const problem=box.querySelector('.aiEditProblem').value.trim(),solution=box.querySelector('.aiEditSolution').value.trim(),confidence=Number(box.querySelector('.aiEditConfidence').value||0);if(!problem||!solution)return err('Problema y solución no pueden quedar vacíos.');try{await api('/api/admin-ai/learning/'+point.id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:point.status,problem,solution,confidence})});await renderSuperAdminAiNeural()}catch(x){err(x.message)}};
  if(del)del.onclick=async()=>{if(!confirm(`Eliminar definitivamente la neurona N${point.id}? Esta acción quita este conocimiento del aprendizaje.`))return;try{await api('/api/admin-ai/learning/'+point.id,{method:'DELETE'});await renderSuperAdminAiNeural()}catch(x){err(x.message)}};
}
...
if(_generalDecorate){decorateDashboardView=function(){_generalDecorate();installGeneralNav();installSuperAdminAiNav();wireSidebarNavigation();wireOverviewShortcuts()}}
const _generalRefresh=typeof psRefreshRoleChrome==='function'?psRefreshRoleChrome:null;
if(_generalRefresh){psRefreshRoleChrome=function(){_generalRefresh();installGeneralNav();installSuperAdminAiNav();wireSidebarNavigation()}}
const _generalShow=show;
show=async function(){const out=await _generalShow();installGeneralNav();installSuperAdminAiNav();wireSidebarNavigation();setTimeout(wireOverviewShortcuts,0);return out};
