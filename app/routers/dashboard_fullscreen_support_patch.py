...
UI_VERSION = '2026.09.02-50'


def _html() -> str:
    html = base_html()
    for old in ('2026.09.02-49','2026.09.02-48','2026.09.02-47','2026.09.02-46'):
...
#decisionFlow:fullscreen{height:100vh!important;max-height:none!important}.support-person-email-grid{display:grid;grid-template-columns:minmax(150px,1fr) minmax(160px,1fr) minmax(220px,1.4fr) auto;gap:8px;align-items:center}.support-person-email-grid button{width:auto}
...
async function installSupportPeopleEmailCard(){
    ...
    try{
        ...
        card.innerHTML=`<div class="section-title"><div><h3>Correos del personal de soporte</h3><div class="muted">Estos correos reciben las alertas de apertura y cierre de tickets.</div></div><button id="testSupportEmail">Enviar correo de prueba</button></div>${rows.map(r=>`<div class="support-person-email-grid" data-support-id="${r.support_id}"><b>${esc(r.name)}</b><span>${esc(r.phone)}</span><input class="supportPersonEmail" type="email" value="${esc(r.email||'')}" placeholder="correo@empresa.com"><button class="saveSupportPersonEmail">Guardar correo</button></div>`).join('')||'<div class="muted">No hay personal de soporte configurado en esta empresa.</div>'}`;
...
companyPanel=async function(key){await _companyPanelFullscreenSupport(key);installTreeFullscreenTools();await installSupportPeopleEmailCard()};
'''
    marker='\n})();'
    if marker in js:
